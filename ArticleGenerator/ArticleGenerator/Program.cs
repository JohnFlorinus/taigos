...
namespace ArticleGenerator
{
    public class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            string newsContent = await ApiService.GetNews();

            Console.WriteLine("Generating Article Essay...");
            string articleEssay = await ApiService.AITextGeneration(
                "Write a concise financial article on the following news summaries. Begin directly with the key facts. Write in a neutral, analytic tone and focus on the important points while avoiding repetitions and irrelevant or minor news items. Do not include any introductory phrases.\n" +
                $"News summaries:\n{newsContent}");

            Console.WriteLine("Generating Article Summary...");
            string articleSummary = await ApiService.AITextGeneration(
                $"Based on the following article, write a concise summary of 2-3 sentences that captures the most significant points. Do not include any introductory phrases and only write the summary directly. Article: {articleEssay}");
            ...
            int newArticleCount = await WebMod.UpdateArticleJS(articleTitle, ApiService.articleImage);
